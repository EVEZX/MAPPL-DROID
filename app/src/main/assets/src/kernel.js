...
class Kernel {
  ...
  appendEvent(event) {
    ...
    const storedEvent = {
      ...
      sequenceNumber: event.sequenceNumber !== undefined ? event.sequenceNumber : this.eventLog.length
    };
    ...
    // Broadcast to Kotlin Native side via the JS interface bridge
    if (window.AndroidKernelBridge && !event._fromAndroid) {
      try {
        window.AndroidKernelBridge.postMessage(JSON.stringify(storedEvent));
      } catch (err) {
        console.error("Failed to forward event to Android bridge:", err);
      }
    }
